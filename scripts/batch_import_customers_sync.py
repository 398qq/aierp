#!/usr/bin/env python3
"""Batch import customers from TYC data (sync version)."""
...
def create_customer(token: str, customer: dict) -> dict:
    """Create a customer, return result."""
    headers = {"Authorization": f"Bearer {token}", "Content-Type": "application/json"}
    resp = requests.post(
        f"{BASE_URL}/api/v1/customers", headers=headers, json=customer, timeout=10
    )
    result = resp.json()
    return {
        "name": customer["name"],
...
def main():
    ...
    for i, cust in enumerate(CUSTOMERS, 1):
        ...
        sys.stdout.flush()
        result = create_customer(token, cust)
