...
import { Col, Progress, Row, Statistic, Tag, Typography, Spin, Alert, List } from "antd";
...
export default function Global360Page() {
  ...
  const scoreColor =
    (insights.enterprise_health_score ?? 0) >= 80
      ? "#52c41a"
      : (insights.enterprise_health_score ?? 0) >= 60
        ? "#faad14"
        : "#ff4d4f";
  ...
  const riskColumns = [
    ...
    { title: "描述", dataIndex: "description", ellipsis: true },
    {
      ...
      width: 80,
      render: (s: string) => (
        <StatusTag status={s} tone={s === "高" ? "danger" : s === "中" ? "warning" : "info"} />
      ),
    },
  ...
  const kpiColumns = [
    ...
    { title: "目标值", dataIndex: "target", width: 100 },
    {
      ...
      render: (s: string) => (
        <StatusTag
          status={s}
          tone={s === "达标" ? "success" : s === "低于" ? "danger" : "warning"}
        />
      ),
    },
  ];
  ...
  const recoColumns = [
    ...
    { title: "建议", dataIndex: "recommendation", ellipsis: true },
    {
      ...
      width: 80,
      render: (p: string) => (
        <StatusTag status={p} tone={p === "高" ? "danger" : p === "中" ? "warning" : "info"} />
      ),
    },
  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      <Row gutter={[16, 16]} style={{ marginBottom: 24 }}>
        <Col xs={24} sm={6}>
          <ProCard>
            <Progress
              type="circle"
              percent={insights.enterprise_health_score}
          ...
          <ProCard title="执行摘要">
            ...
            {insights.focus_areas?.length > 0 && (
              <div style={{ marginTop: 12 }}>
                ...
                {insights.focus_areas.map((f: string, i: number) => (
                  <StatusTag status={f} tone="info" key={i} />
                ))}
              </div>
            )}
          </ProCard>
      ...
      {insights.kpi_health?.length > 0 && (
        <ProCard title="KPI 健康看板" style={{ marginBottom: 24 }}>
          <ProTable
            columns={kpiColumns as any}
            dataSource={insights.kpi_health}
      ...
      <Row gutter={[16, 16]} style={{ marginBottom: 24 }}>
        <Col span={12}>
          ...
          >
            <ProTable
              columns={oportunidadColumns as any}
              dataSource={insights.top_opportunities}
              rowKey={(_r, i) => String(i)}
              pagination={false}
              search={false}
              options={false}
        ...
        <Col span={24}>
          ...
          >
            {insights.cross_domain_correlations?.length > 0 ? (
              <List
                size="small"
                dataSource={insights.cross_domain_correlations}
                renderItem={(item: any) => (
                  <List.Item>
      ...
      <ProCard title="战略建议">
        <ProTable
          columns={recoColumns as any}
          dataSource={insights.strategic_recommendations}
          rowKey={(_r, i) => String(i)}
          pagination={false}
          search={false}
          options={false}
      ...
      {raw && (
        <ProCard title="原始数据快照" style={{ marginTop: 24 }} size="small">
