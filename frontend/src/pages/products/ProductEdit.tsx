...
import type { APIResponse, Brand, Product, Warehouse } from "../../types";
...
export default function ProductEdit() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      client.get<APIResponse<PackLevel[]>>(`/products/${productId}/pack-levels`).catch(() => null),
    ])
      .then(([productResponse, brandResponse, warehouseResponse, packResp]) => {
        ...
        const loaded = (packResp?.data.data || []) as PackLevel[];
        if (loaded.length) {
          setPackLevels(loaded);
        } else {
          // Default: level 0 from product unit
          setPackLevels([
            { pack_level: 0, uom_code: nextProduct.unit || "pcs", qty_per_parent: 1 },
          ]);
        }
      })
  ...
  const handleSave = async (values: Record<string, unknown>) => {
    ...
    try {
      ...
      // Save pack levels
      const validLevels = packLevels.filter((p) => p.uom_code);
      if (validLevels.length) {
        await client.put(`/products/${productId}/pack-levels`, validLevels);
      }
          ...
          <Table
            ...
            columns={[
              ...
              },
              {
                title: "单位",
                width: 160,
                render: (_: unknown, r: PackLevel, idx: number) => (
                  <UomSelect
                    uomType={r.pack_level === 0 ? "count" : "package"}
                    value={r.uom_code}
                    onChange={(v) => {
                      const next = [...packLevels];
                      next[idx] = { ...r, uom_code: v || "" };
                      setPackLevels(next);
                    }}
                  />
                ),
              ...
              },
              {
                ...
                render: (_: unknown, r: PackLevel) => {
                  ...
                  return (
                    <Typography.Text type="secondary">
                      1 {r.uom_code || "?"} = {r.qty_per_parent} {parentLabel}
                    </Typography.Text>
                  );
                },
              },
