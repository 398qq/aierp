...
function matchField(colName: string, field: string): boolean {
  ...
  return aliases.some((a) => normalized.toLowerCase() === a.toLowerCase() || normalized.includes(a.toLowerCase()));
}
...
export default function PriceImport() {
  ...
  const parseFile = useCallback(async (file: File) => {
    ...
    const json = XLSX.utils.sheet_to_json<string[]>(firstSheet, { header: 1 }) as string[][];
    if (json.length < 2) {
  ...
  const handleMappingConfirm = () => {
    ...
    const parsed: ParsedRow[] = rawData.slice(0, 10).map((row) => ({
      ...
      unit_price: priceIdx >= 0 ? parseFloat(row[priceIdx]) || null : null,
  ...
  const handleImport = async () => {
    ...
    const items = rawData.map((row) => ({
      sku: String(row[skuIdx] ?? "").trim(),
      warehouse_id: normalizeWarehouse(row[whIdx]),
      unit_price: priceIdx >= 0 ? parseFloat(row[priceIdx]) || 0 : 0,
      ...
      {step === "upload" && (
        <Card>
          <Dragger {...uploadProps} style={{ padding: "40px 0" }}>
            ...
            <p style={{ color: "#888" }}>支持 .xlsx、.xls、.csv 格式</p>
          </Dragger>
        </Card>
      )}
