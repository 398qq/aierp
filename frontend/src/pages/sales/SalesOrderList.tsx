...
import { getSalesOrders, batchDeleteSalesOrders, deleteSalesOrder, convertSalesOrderToDelivery } from "../../api";
...
export default function SalesOrderList() {
  ...
  const load = async () => {
    ...
    try {
      ...
      if (includeAi) setAiMap((resp.data.data as unknown as Record<string, unknown>).ai as Record<string, { delivery_risk?: string; flag?: string }> || {});
      else setAiMap({});
  ...
  const handleBatchDelete = async () => {
    try { await batchDeleteSalesOrders(selected); message.success("已批量删除"); setSelected([]); load(); } catch { message.error("删除失败"); }
  };

  return (
    <div>
      <Space style={{ marginBottom: 16 }}>
        <Button type="primary" icon={<PlusOutlined />} onClick={() => navigate("/sales/orders/new")}>新增订单</Button>
        {selected.length > 0 && (
      ...
      <Table
        ...
        columns={[
          { title: "订单号", dataIndex: "order_no", width: 140, render: (v: string, r: SalesOrder) => <a onClick={() => navigate(`/sales/orders/${r.id}`)}>{v || `#${r.id}`}</a> },
          ...
          { title: "下单日期", dataIndex: "order_date", width: 110, render: (v: string) => v?.slice(0, 10) || "-" },
          { title: "预计交货", dataIndex: "delivery_date", width: 110, render: (v: string) => v?.slice(0, 10) || "-" },
          {
            title: "AI", width: 90,
            render: (_: unknown, r: SalesOrder) => <AIInlineBadge riskLevel={aiMap[r.id]?.delivery_risk} flag={aiMap[r.id]?.flag} />,
          },
          {
            title: "操作", width: 200,
            render: (_: unknown, r: SalesOrder) => (
              <Space size="small">
                <Button size="small" onClick={() => navigate(`/sales/orders/${r.id}`)}>详情</Button>
                <Popconfirm title="转为发货单?" onConfirm={async () => {
                  try { await convertSalesOrderToDelivery(r.id); message.success("已转为发货单"); load(); } catch { message.error("转换失败"); }
                }}><Button size="small" type="primary">发货</Button></Popconfirm>
                <Popconfirm title="确定删除?" onConfirm={async () => {
                  try { await deleteSalesOrder(r.id); message.success("已删除"); load(); } catch { message.error("删除失败"); }
