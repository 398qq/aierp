...
import { SearchOutlined, CheckCircleOutlined, CalculatorOutlined } from "@ant-design/icons";
...
export default function InventoryBatches() {
    ...
    if (picks.length > 0) {
      ...
      try {
        ...
        setPreviewResult({
          ...
          weighted_unit_cost: picks.length
            ? totalCost / picks.reduce((s, p) => s + p.quantity, 0)
            : 0,
          unfilled_qty: 0,
  ...
  const batchColumns: ProColumns<InventoryBatch>[] = [
    { title: "批次号", dataIndex: "batch_no", width: 140 },
    {
      ...
      width: 160,
      ellipsis: true,
      render: (v, r) => (v as string) || `产品 #${r.product_id}`,
    },
    {
      title: "供应商",
    ...
    },
    {
      ...
      width: 120,
      align: "right",
      render: (_, r) => money(r.quantity * r.unit_cost),
    },
    { title: "入库日期", dataIndex: "received_date", width: 110 },
    {
      ...
      dataIndex: "status",
      width: 80,
      render: (v) => <Tag color={STATUS_COLORS[v as string] || "default"}>{v as string}</Tag>,
    },
  ];
    ...
    <Space direction="vertical" size={16} style={{ width: "100%" }}>
      ...
      >
        <ProTable<InventoryBatch>
          ...
          request={async (params) => {
            const r = await getInventoryBatches({
              page: params.current,
              page_size: params.pageSize,
              status: statusFilter || "available",
            });
            const list = (r.data.data?.list || []) as InventoryBatch[];
            const total = (r.data.data?.total || 0) as number;
            return { data: list, success: true, total };
          }}
          onLoad={(ds) => setBatches(ds as InventoryBatch[])}
      ...
      >
        <Space direction="vertical" size={12} style={{ width: "100%" }}>
          <Radio.Group
            value={Object.keys(manualPicks).length > 0 ? "manual" : strategy}
            onChange={(e) => {
          ...
          {previewResult && (
            <>
              ...
              <Space>
                ...
                <Text type="secondary">加权单价: {money(previewResult.weighted_unit_cost)}</Text>
                {!previewResult.is_fully_allocated && (
                  <Text type="danger">库存不足！缺 {previewResult.unfilled_qty} 件</Text>
                )}
              </Space>
            </>
          )}
