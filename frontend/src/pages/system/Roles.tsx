...
import { Table, Button, Space, Modal, Form, Input, Tree, message, Popconfirm, Card, Tag, Typography } from "antd";
...
interface PermGroup { resource: string; resource_label: string; actions: { id: number; action: string; name: string }[]; }
...
export default function Roles() {
  ...
  const fetchRoles = async () => {
    ...
    try {
      ...
      const grouped: PermGroup[] = Object.entries(groups).map(([res, actions]) => ({
        resource: res,
        resource_label: ({ customers: "客户", products: "产品", sales: "销售", purchases: "采购", finance: "财务", inventory: "库存", reports: "报表", system: "系统" } as Record<string, string>)[res] || res,
        actions: (actions as { id: number; action: string; name: string }[]),
      }));
  ...
  const treeData = permGroups.map((g) => ({
    ...
    children: g.actions.map((a) => ({ key: a.id, title: a.name })),
  }));
  ...
  const columns: ColumnsType<Role> = [
    ...
    { title: "权限数", key: "pc", width: 80, render: (_, r) => r.permission_ids?.length || 0 },
    {
