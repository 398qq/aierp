      ...
      {loading ? (
        ...
        <Space direction="vertical" size={12} style={{ width: "100%" }}>
          ...
          {!stats ? (
            ...
            <Row gutter={[10, 10]}>
              <Col span={8}><Card size="small"><Statistic title="订单数" value={stats.order_count} /></Card></Col>
              <Col span={8}><Card size="small"><Statistic title="总营收" value={stats.total_revenue} precision={2} /></Card></Col>
              <Col span={8}><Card size="small"><Statistic title="信用占用%" value={stats.credit_usage_pct} precision={1} /></Card></Col>
            </Row>
          )}

          <Divider style={{ margin: "4px 0" }}>快速动作</Divider>
          <Space wrap>
            <Button icon={<EyeOutlined />} onClick={() => navigate(`/customers/${customer.id}`)}>完整详情</Button>
            <Button icon={<ShoppingCartOutlined />} onClick={() => navigate(`/sales/orders/new?customer_id=${customer.id}`)}>建订单</Button>
            <Button icon={<PhoneOutlined />} onClick={() => navigate(`/customers/${customer.id}?tab=followups`)}>建跟进</Button>
            <Button icon={<SwapOutlined />} onClick={() => onVendAsSupplier(customer)}>转供应商</Button>
          </Space>
        </Space>
      )}
    </Drawer>
