...
const isOpenFollowUp = (item: FollowUp) =>
...
export default function CustomerDetail() {
  ...
  const nextOpenFollowUp = useMemo(
    () =>
      followUps
        ...
        .sort(
          (a, b) =>
            new Date(a.planned_at || a.created_at).getTime() -
            new Date(b.planned_at || b.created_at).getTime(),
        )[0],
    [followUps],
  );
  ...
  const handleUnlinkTag = async (tagId: number) => {
    try {
      await unlinkTag(customerId, tagId);
      loadTags();
    } catch (e: unknown) {
      message.error(getApiErrorMessage(e, "移除标签失败"));
    }
  };

  const handleLinkTag = async (tagId: number) => {
    try {
      await linkTag(customerId, tagId);
      loadTags();
      message.success("标签已添加");
    } catch (e: unknown) {
      message.error(getApiErrorMessage(e, "添加标签失败"));
    }
  };
  ...
  const handleGenerateDefaultTags = async () => {
    ...
    setAllTags(
      ((tagsResp.data.data as TagType[]) || []).sort((a, b) => a.name.localeCompare(b.name)),
    );
  ...
  const handleProductRecs = async () => {
    ...
    try {
      ...
      if (resp.data.code === 0) {
        setRecResult(resp.data.data as CustomerProductMatch);
        setRecModalOpen(true);
      }
    } catch (e: unknown) {
      message.error(getApiErrorMessage(e, "AI 推荐失败"));
    } finally {
      setRecLoading(false);
    }
  };
          ...
          <Card
            ...
            title={
              <div className="customer-detail-title">
                ...
                <StatusTag tone={getLevelColor(customer.level)}>
                  等级 {customer.level || "-"}
                </StatusTag>
            ...
            <div className="customer-detail-layout">
              <section className="customer-detail-panel">
                ...
                <Descriptions column={1} size="small">
                  ...
                  <Descriptions.Item label="最近联系">
                    {formatShortDateTime(customer.last_contacted_at)}
                  </Descriptions.Item>
                </Descriptions>
              ...
              <section className="customer-detail-panel">
                <div className="customer-detail-panel-title">
                  ...
                  <Button
                    size="small"
                    onClick={() => navigate(`/customers/${customerId}/follow-ups`)}
                  >
                    全部跟进
                  </Button>
                </div>
                <div className="customer-detail-next">
                  {nextOpenFollowUp ? (
                    <Space direction="vertical" size={5} style={{ width: "100%" }}>
                      <Space wrap>
                        <StatusTag tone={getFollowUpDueMeta(nextOpenFollowUp).color}>
                          {getFollowUpDueMeta(nextOpenFollowUp).text}
                        </StatusTag>
                      ...
                      <Typography.Text strong>
                        {formatShortDateTime(nextOpenFollowUp.planned_at)}
                      </Typography.Text>
                      <Typography.Text type="secondary">
                        {nextOpenFollowUp.content || "暂无跟进内容"}
                      </Typography.Text>
                    </Space>
                  ) : (
                    ...
                    return (
                      <div className="customer-detail-follow-item" key={item.id}>
                        <Space wrap size={6}>
                          ...
                          <Typography.Text type="secondary">
                            {formatShortDateTime(item.planned_at)}
                          </Typography.Text>
                        </Space>
              ...
              <aside className="customer-detail-panel is-action">
                ...
                <div className="customer-detail-action-grid">
                  ...
                  <Button icon={<BulbOutlined />} loading={recLoading} onClick={handleProductRecs}>
                    AI 产品推荐
                  </Button>
                  <Button
                    icon={<PieChartOutlined />}
                    onClick={() => navigate(`/customers/${customerId}/360`)}
                  >
                    AI 360 洞察
                  </Button>
                      ...
                      <Card
                        ...
                        actions={[
                          ...
                          <Popconfirm
                            ...
                            onConfirm={async () => {
                              try {
                                await deleteContact(customerId, c.id);
                                message.success("已删除");
                                load();
                              } catch (e: unknown) {
                      ...
                      >
                        <Descriptions column={3} size="small">
                          <Descriptions.Item label="姓名">
                            {c.name}
                            {c.is_primary && (
                              <StatusTag tone="processing" style={{ marginLeft: 4 }}>
                                主要
                              </StatusTag>
                            )}
                          </Descriptions.Item>
                      ...
                      <Card
                        ...
                        actions={[
                          ...
                          <Popconfirm
                            ...
                            onConfirm={async () => {
                              try {
                                await deleteFollowUp(customerId, f.id);
                                message.success("已删除");
                                load();
                              } catch (e: unknown) {
                                message.error(getApiErrorMessage(e, "删除失败"));
                      ...
                      >
                        <Descriptions column={3} size="small">
                          <Descriptions.Item label="提醒状态">
                            <StatusTag tone={getFollowUpDueMeta(f).color}>
                              {getFollowUpDueMeta(f).text}
                            </StatusTag>
                          </Descriptions.Item>
                          <Descriptions.Item label="方式">
                            <FollowUpMethodTag method={f.method} />
                          </Descriptions.Item>
                          <Descriptions.Item label="状态">
                            <FollowUpStatusTag status={f.status} />
                          </Descriptions.Item>
                          <Descriptions.Item label="优先级">
                            <FollowUpPriorityTag priority={f.priority} />
                          </Descriptions.Item>
                          <Descriptions.Item label="计划时间">
                            {formatShortDateTime(f.planned_at)}
                          </Descriptions.Item>
                          <Descriptions.Item label="负责人">
                            {f.assigned_to || "-"}
                          </Descriptions.Item>
                        ...
                        {f.result && (
                          <p style={{ marginTop: 4, color: "#666" }}>结果：{f.result}</p>
                        )}
                      </Card>
                    ))}
                  </div>
          ...
          >
            {recResult && (
              <div>
                ...
                <Table
                  size="small"
                  dataSource={recResult.recommendations}
                  rowKey="product_name"
                  pagination={false}
                  columns={[
                    ...
                    { title: "预计价值", dataIndex: "estimated_potential", width: 180 },
                    {
                      ...
                      width: 80,
                      render: (v: string) => (
                        <StatusTag tone={v === "高" ? "danger" : v === "中" ? "info" : "neutral"}>
                          {v}
                        </StatusTag>
                      ),
                    },
                ...
                <Card size="small" type="inner" style={{ marginTop: 12, background: "#fffbe6" }}>
                  ...
                  <Typography.Text>{recResult.approach_strategy}</Typography.Text>
                </Card>
              </div>
            )}
          </Modal>
      ...
      <Col xs={24} lg={8}>
        <Card size="small" title="下一步动作">
          <Space direction="vertical" size={10} style={{ width: "100%" }}>
            ...
            <Space wrap>
              <Button type="primary" icon={<PhoneOutlined />} onClick={onNewFollowUp}>
                建跟进
              </Button>
              ...
              renderItem={(item) => (
                <List.Item>
                  <List.Item.Meta
                    title={
                      <Space wrap>
                        <StatusTag tone={getFollowUpDueMeta(item).color}>
                          {getFollowUpDueMeta(item).text}
                        </StatusTag>
  ...
  return (
    <div>
      <Row gutter={[16, 16]}>
        ...
        <Col xs={12} sm={6}>
          <Card size="small">
            <Statistic title="当前状态" value={stats.lifecycle} />
            <div style={{ marginTop: 8, color: "#888", fontSize: 12 }}>
              创建 {stats.created_days} 天
            </div>
          </Card>
        </Col>
        <Col xs={12} sm={6}>
            ...
            {agingEntries.map(([bucket, amt]) => (
              <Col xs={12} sm={6} key={bucket}>
                <Statistic
                  ...
                  prefix="¥"
                  valueStyle={{
                    color: bucket === "90+" ? "#cf1322" : bucket === "60-90" ? "#fa8c16" : "#333",
                  }}
                />
              </Col>
            ))}
      ...
      {stats.last_order_date && (
        <div style={{ marginTop: 12, color: "#888", fontSize: 12 }}>
          最近订单: {stats.last_order_date?.slice(0, 10)} | 已付金额: ¥
          {stats.paid_total.toLocaleString()}
        </div>
      )}
    </div>
  );
      ...
      <Space direction="vertical" size={16} style={{ width: "100%" }}>
        {customerTags.length > 0 && (
          <div>
            ...
            {customerTags.map((t) => (
              <StatusTag key={t.id} tone={t.color || "info"}>
                {t.name}
              </StatusTag>
            ))}
          </div>
        ...
        <div>
          ...
          {available.length === 0 && (
            <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="没有更多可用的标签" />
          )}
          <Space wrap>
            {available.map((t) => (
              <StatusTag
                key={t.id}
                tone={t.color || "neutral"}
    ...
    <Timeline
      items={logs.map((l) => ({
        ...
        children: (
          <div>
            <Space size={4}>
              <StatusTag tone={ACTION_COLORS[l.action]}>
...
function GroupPanel({ customerId, customerName }: { customerId: number; customerName: string }) {
  ...
  const handleLink = async () => {
    if (!parentId) return;
    try {
      await linkParent(customerId, parentId);
  ...
  return (
    <div>
      <Space style={{ marginBottom: 16 }}>
        ...
        <Button icon={<DisconnectOutlined />} onClick={handleUnlink}>
          解除关联
        </Button>
      </Space>
      ...
      {stats && (
        <Row gutter={[16, 16]} style={{ marginBottom: 16 }}>
          <Col xs={12} sm={6}>
            <Card size="small">
              <Statistic title="集团客户数" value={stats.members} />
            </Card>
          </Col>
          <Col xs={12} sm={6}>
            <Card size="small">
              <Statistic title="集团订单数" value={stats.agg_orders} />
            </Card>
          </Col>
          <Col xs={12} sm={6}>
            <Card size="small">
              <Statistic title="集团营收" value={stats.agg_revenue} prefix="¥" />
            </Card>
          </Col>
          <Col xs={12} sm={6}>
            <Card size="small">
              <Statistic title="集团信用额" value={stats.agg_credit} prefix="¥" />
            </Card>
          </Col>
        </Row>
      )}
        ...
        {children.length === 0 ? (
          ...
          <List
            ...
            renderItem={(c) => (
              <List.Item
                extra={
                  <StatusTag tone={c.level === "A" ? "danger" : "neutral"}>{c.level}</StatusTag>
                }
              >
                <List.Item.Meta
      ...
      columns={[
        {
          ...
          dataIndex: "name",
          key: "name",
          render: (name: string, r: SimilarCustomer) => <a href={`/customers/${r.id}`}>{name}</a>,
        },
        {
          title: "行业",
          dataIndex: "industry",
...
function VisitPanel({ customerId }: { customerId: number }) {
  ...
  const openForm = (v?: Visit) => {
    ...
    if (v)
      form.setFieldsValue({
        ...
        followup_date: v.followup_date?.slice(0, 10),
      });
    else form.resetFields();
    setModalOpen(true);
  };
  ...
  const handleDelete = async (id: number) => {
    try {
      await deleteCustomerVisit(customerId, id);
      message.success("已删除");
      load();
    } catch (e: unknown) {
      message.error(getApiErrorMessage(e, "删除失败"));
  ...
  return (
    <div>
      <Button type="primary" onClick={() => openForm()} style={{ marginBottom: 16 }}>
        新增拜访
      </Button>
      {loading ? (
        <Spin />
      ) : visits.length === 0 ? (
        <Empty description="暂无拜访记录" />
      ) : (
        <List
          dataSource={visits}
          renderItem={(v) => (
            ...
            >
              <Row gutter={8}>
                <Col span={6}>
                  <Typography.Text strong>{v.title || "无标题"}</Typography.Text>
                </Col>
                <Col span={4}>
                  <StatusTag>{TYPE[v.type as string] || v.type}</StatusTag>
                </Col>
                <Col span={4}>
                  <StatusTag tone={STATUS[v.status as string]?.color}>
                    {STATUS[v.status as string]?.label || v.status}
                  </StatusTag>
                </Col>
              ...
              {v.content && (
                <Typography.Paragraph ellipsis={{ rows: 2 }} style={{ marginTop: 8, fontSize: 13 }}>
                  {v.content}
                </Typography.Paragraph>
              )}
            </Card>
      ...
      >
        <Form form={form} layout="vertical" onFinish={onFinish}>
          ...
          <Row gutter={12}>
            ...
            <Col span={8}>
              <Form.Item name="visit_date" label="日期">
          ...
          <Form.Item name="content" label="拜访内容">
            <Input.TextArea rows={3} />
          </Form.Item>
          <Form.Item name="result" label="拜访结果">
            <Input.TextArea rows={2} />
          </Form.Item>
