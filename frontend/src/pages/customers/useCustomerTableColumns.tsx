...
import { formatDate, formatDateTime, getHealthColor, getLevelColor } from "./constants";
    ...
    () => [
      {
        ...
        render: (v: string | null) => <Typography.Text style={MONO}>{v || "-"}</Typography.Text>,
      },
      {
        title: "客户名称",
        dataIndex: "name",
        ...
        render: (text: string, r: Customer) => (
          <div>
            <Space size={4}>
              ...
              {overdueCustomerIds.has(r.id) && <StatusTag tone="danger">逾期</StatusTag>}
            </Space>
            <div style={{ color: "#8c8c8c", fontSize: 12, marginTop: 1 }}>
              {r.short_name || r.code
                ? `${r.short_name || r.code}${r.tax_id ? ` · ${r.tax_id}` : ""}`
                : "-"}
            </div>
          </div>
        ),
      ...
      },
      {
        ...
        render: (v: string | null) => <StatusTag tone={getLevelColor(v)}>{v || "-"}</StatusTag>,
      },
      {
        title: "区域",
        dataIndex: "region",
      ...
      },
      {
        ...
        render: (_: unknown, r: Customer) => (
          <Space size={4}>
            {r.credit_level ? <StatusTag>{r.credit_level}</StatusTag> : "-"}
      ...
      },
      {
        ...
        key: "payment_terms",
        width: 110,
        render: (v: string | null) => (v ? <StatusTag tone="info">{v}</StatusTag> : "-"),
      },
      {
        title: "币种",
        dataIndex: "currency",
        ...
        width: 64,
        align: "center",
        render: (v: string) => <Typography.Text style={MONO}>{v || "CNY"}</Typography.Text>,
      },
      {
        title: "税号",
        dataIndex: "tax_id",
      ...
      },
      {
        ...
        render: (v: string | null) => {
          if (!v) return <Typography.Text type="secondary">从未</Typography.Text>;
          const days = Math.floor((Date.now() - new Date(v).getTime()) / 86400000);
          if (days === 0) return <span style={{ color: "#52c41a" }}>今天</span>;
          if (days === 1) return <span style={{ color: "#1677ff" }}>昨天</span>;
          if (days <= 7)
            return <span style={{ color: days > 5 ? "#faad14" : "#1677ff" }}>{days}天前</span>;
          if (days <= 30) return <span style={{ color: "#faad14" }}>{days}天前</span>;
          return <span style={{ color: "#ff4d4f" }}>{Math.floor(days / 30)}月前</span>;
        },
      },
      {
        title: "来源",
