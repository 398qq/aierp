...
export const getProduct = (id: number) => client.get<APIResponse<Product>>(`/products/${id}`);
...
export const deleteProduct = (id: number) => client.delete<APIResponse>(`/products/${id}`);
...
export const priceImport = (
  items: { sku: string; warehouse_id: number; unit_price?: number; quantity?: number }[],
) =>
  client.post<APIResponse<{ success: number; failed: number; errors: string[] }>>(
...
export const getPricingRecommend = (params: {
  product_id: number;
  customer_id?: number;
  quantity?: number;
  is_sample?: boolean;
...
export const normalizeProductSpecs = (productId: number) =>
  client.post<APIResponse<{ parameters: NormalizedSpec[]; raw: string }>>(
    `/ai/products/${productId}/normalize-specs`,
  );

export const getProductAssociations = (productId: number, topK = 10) =>
  client.get<APIResponse<{ associations: ProductAssociation[]; target_product_id: number }>>(
    `/ai/products/${productId}/associations?top_k=${topK}`,
  );

export const getProcurementOptimize = (productId: number, quantity: number) =>
  client.post<APIResponse<ProcurementPlan>>(
    `/ai/products/${productId}/procurement-optimize?quantity=${quantity}`,
  );
...
export const createWarehouse = (data: { name: string; location?: string; description?: string }) =>
  client.post<
    APIResponse<{ id: number; name: string; location: string | null; description: string | null }>
  >("/warehouses", undefined, { params: data });
...
export const updateWarehouse = (
  ...
  client.put<
    APIResponse<{ id: number; name: string; location: string | null; description: string | null }>
  >(`/warehouses/${id}`, undefined, { params: data });
...
export const adjustInventory = (
  ...
  client.post<APIResponse>(
    `/inventory/adjust?product_id=${product_id}&warehouse_id=${warehouse_id}&adjustment=${adjustment}&reason=${encodeURIComponent(reason)}`,
  );

export const batchAdjustInventory = (
  items: { product_id: number; warehouse_id: number; adjustment: number; reason: string }[],
...
export const createProductInventory = (data: {
  product_id: number;
  warehouse_id: number;
  quantity: number;
  safety_stock?: number;
...
export const updateProductInventory = (
  id: number,
  data: { quantity?: number; safety_stock?: number; unit_price?: number },
...
export const embedProduct = (id: number) => client.post<APIResponse>(`/ai/products/${id}/embed`);

export const embedAllProducts = () => client.post<APIResponse>("/ai/products/embed-all");
...
// Inventory batches & COGS
export const getInventoryBatches = (params: Record<string, unknown>) =>
  client.get<APIResponse<{ list: import("../types").InventoryBatch[]; total: number }>>(
    "/inventory/batches", { params }
  );

export const previewAllocation = (data: { product_id: number; warehouse_id: number; quantity: number }) =>
  client.post<APIResponse<{
  ...
  }>>("/inventory/batches/allocate", data);

export const getInventoryCogs = (start_date?: string, end_date?: string) =>
  client.get<APIResponse<import("../types").CogsReport>>("/inventory/cogs", {
    params: { start_date, end_date },
  });

export const commitBatchAllocation = (data: {
  product_id: number;
  warehouse_id: number;
  picks: { batch_id: number; quantity: number }[];
}) =>
  client.post<APIResponse<{
    total_cogs: number;
    allocations: { batch_id: number; batch_no: string; quantity: number; unit_cost: number; line_cost: number }[];
  }>>("/inventory/batches/commit", data);
