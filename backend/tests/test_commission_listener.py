...
async def test_on_invoice_paid_creates_commission(db: AsyncSession):
    ...
    db.add(cust)
    order = SalesOrder(
        id=10, order_no="SO-001", customer_id=1, status="completed", total_amount=10000
    )
    db.add(order)
    inv = Invoice(
        id=20,
...
async def test_idempotent_no_double_commission(db: AsyncSession):
    ...
    db.add(cust)
    order = SalesOrder(
        id=20, order_no="SO-002", customer_id=2, status="completed", total_amount=5000
    )
    db.add(order)
    inv = Invoice(
        id=30,
    ...
    # DB has exactly one
    count = (
        (await db.execute(select(Commission).where(Commission.sales_order_id == 20)))
        .scalars()
        .all()
    )
    assert len(count) == 1
...
async def test_skips_unpaid_invoice(db: AsyncSession):
    ...
    db.add(cust)
    order = SalesOrder(
        id=30, order_no="SO-003", customer_id=4, status="pending", total_amount=1000
    )
    db.add(order)
    inv = Invoice(
        id=50,
...
async def test_skips_when_owner_not_numeric(db: AsyncSession):
    ...
    db.add(cust)
    order = SalesOrder(
        id=40, order_no="SO-004", customer_id=5, status="completed", total_amount=2000
    )
    db.add(order)
    inv = Invoice(
        id=60,
