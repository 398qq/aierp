...
class TestEmbed:
    ...
    async def test_embed_single_delegates(self):
        ...
        with patch.object(client, "embed", new=AsyncMock(return_value=[[0.1, 0.2]])) as mock_embed:
...
class TestChatStructured:
    ...
    async def test_chat_structured_adds_schema_to_prompt(self):
        ...
        client = AIClient()
        messages = [{"role": "system", "content": "Be helpful"}, {"role": "user", "content": "query"}]
        schema = {"type": "object", "properties": {"name": {"type": "string"}}}

        # We just verify the messages are modified correctly before calling chat
        with patch.object(client, "chat", new=AsyncMock(return_value='{"name": "test"}')):
    ...
    async def test_chat_structured_coerces_numbers(self):
        ...
        with patch.object(client, "chat", new=AsyncMock(return_value='{"count": "42"}')):
            result = await client.chat_structured(messages, schema)
