...
async def setup_commissions(db_session: AsyncSession):
    ...
    for i in range(1, 4):
        c = Commission(
            commission_no=f"CM-B{i}", sales_order_id=i, sales_user_id=1,
...
async def test_batch_approve_mixed_success_and_failure(setup_commissions):
    ...
    with patch("app.services.cache_service.cache_bump_version", new_callable=AsyncMock):
        result = await batch_transition(
            payload={"ids": [1, 2, 3, 9999], "to": "approved", "notes": "monthly batch"},
            db=db, user=user,
        )
...
async def test_batch_empty_ids_returns_400(db_session: AsyncSession):
    user = {"id": 42, "username": "manager"}
    result = await batch_transition(
        payload={"ids": [], "to": "approved"}, db=db_session, user=user,
    )
...
async def test_batch_invalid_target_returns_400(db_session: AsyncSession):
    user = {"id": 42, "username": "manager"}
    result = await batch_transition(
        payload={"ids": [1], "to": "invalid_status"}, db=db_session, user=user,
    )
    assert result["code"] == 400
...
async def test_batch_paid_with_amount(db_session: AsyncSession):
    ...
    for i in range(3):
        c = Commission(
            commission_no=f"CM-P{i}", sales_order_id=i, sales_user_id=1,
    ...
    with patch("app.services.cache_service.cache_bump_version", new_callable=AsyncMock):
        result = await batch_transition(
            payload={
                "ids": ids, "to": "paid",
                "paid_amount": 100.0, "notes": "May payout",
            },
    ...
    rows = (await db_session.execute(
        select(Commission).where(Commission.id.in_(ids))
    )).scalars().all()
    for r in rows:
...
async def test_batch_bumps_cache_once(db_session: AsyncSession):
    ...
    with patch("app.services.cache_service.cache_bump_version", new_callable=AsyncMock) as mock_bump:
