...
class TestProductsAPI:
    ...
    async def test_list_products_searches_brand_specs_and_package(self, async_client: AsyncClient, auth_headers: dict):
        brand_resp = await async_client.post(
            "/api/v1/brands/",
            headers=auth_headers,
            json={"name": "Searchable Brand", "name_cn": "可搜索品牌", "status": "active"},
        )
        ...
        for query in ["可搜索品牌", "low noise", "QFN-32", "模拟前端"]:
            resp = await async_client.get(f"/api/v1/products?q={query}", headers=auth_headers)
    ...
    async def test_product_row_includes_management_metrics(self, async_client: AsyncClient, auth_headers: dict, db_session):
        ...
        product_id = create_resp.json()["data"]["id"]
        db_session.add_all([
            Inventory(product_id=product_id, warehouse_id=wh1.id, quantity=10, safety_stock=5, locked_quantity=2, unit_price=1.2),
            Inventory(product_id=product_id, warehouse_id=wh2.id, quantity=3, safety_stock=5, locked_quantity=0, unit_price=1.4),
            SupplierProduct(product_id=product_id, supplier_id=supplier.id, cost_price=1.0),
        ])
        await db_session.flush()
        ...
        assert data["completion_score"] == 83
        assert data["missing_fields"] == ["品牌"]

    async def test_batch_delete_products(self, async_client: AsyncClient, auth_headers: dict):
        create_resp = await async_client.post("/api/v1/products", headers=auth_headers, json={
        ...
        resp = await async_client.post("/api/v1/products/batch-delete", headers=auth_headers, json={"ids": [product_id]})
        assert resp.status_code == 200
        assert resp.json()["data"]["deleted"] == 1

        missing_resp = await async_client.get(f"/api/v1/products/{product_id}", headers=auth_headers)
    ...
    async def test_brand_search_q_and_management_fields(
        ...
        for query in ["搜索品牌", "Power Management", "industrial control"]:
            resp = await async_client.get(f"/api/v1/brands/?q={query}", headers=auth_headers)
            assert resp.status_code == 200
    ...
    async def test_brand_scene_filters_and_sorting(
        ...
        high_resp = await async_client.post(
            "/api/v1/brands/",
            headers=auth_headers,
            json={"name": "High Risk Brand", "status": "active", "risk_level": "high", "risk_score": 90},
        )
        stocked_resp = await async_client.post(
            "/api/v1/brands/",
            headers=auth_headers,
            json={"name": "Stocked Brand", "status": "active", "risk_level": "low", "risk_score": 10},
        )
        ...
        high_risk_resp = await async_client.get("/api/v1/brands/?scene=high_risk", headers=auth_headers)
        ...
        no_products_resp = await async_client.get("/api/v1/brands/?scene=no_products", headers=auth_headers)
        ...
        sorted_resp = await async_client.get("/api/v1/brands/?sort=product_count_desc", headers=auth_headers)
    ...
    async def test_brand_batch_update_and_delete_contract(
        ...
        positioning_resp = await async_client.patch(
            "/api/v1/brands/batch",
            headers=auth_headers,
            json={"ids": [brand_id], "updates": {"positioning": "high", "rohs_status": "compliant"}},
        )
        assert positioning_resp.status_code == 200
        assert positioning_resp.json()["data"]["fields"] == ["positioning", "rohs_status"]
