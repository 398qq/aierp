...
async def recommend_products_for_customer(
    db: AsyncSession, customer_id: int, top_k: int = 5
) -> dict:
        ...
        bought_profile = (
            await db.execute(
                select(
                    ...
                    func.string_agg(func.distinct(Product.category), ", ").label(
                        "cats"
                    ),
                )
        ...
        sc_rows = (
            await db.execute(
                select(
                    ...
                    func.count(func.distinct(SalesOrderItem.product_id)).label(
                        "shared"
                    ),
                )
    ...
    if candidate_pids:
        ...
        candidates = [
            {
                "product_id": r[0],
                "product_name": f"{r[2] or ''} {r[1]}".strip(),
                "category": r[3],
                "brand": r[4] or r[5] or "未知",
                "times_bought": r[6],
            }
            for r in cand_rows
        ]
    ...
    ai_result = await ai_client.chat_structured(
        [
            {
                "role": "system",
                "content": "你是一个电子元器件销售策略专家，擅长基于客户画像推荐产品。",
            },
            {
                "role": "user",
                "content": customer_product_matching_prompt(
                    customer_profile, candidates_text
                ),
            },
        ],
        schema,
    ...
    ai_result["customer_profile"] = {
        ...
        "bought_brands": bought_profile[0] or "",
        "bought_categories": bought_profile[1] or "",
...
async def recommend_customers_for_product(
    db: AsyncSession, product_id: int, top_k: int = 5
) -> dict:
    ...
    if len(target_ids) < top_k:
        ...
        extra = [
            c
            for c in cat_customers
            if c not in existing_customer_ids and c not in target_ids
        ]
    ...
    ai_result = await ai_client.chat_structured(
        [
            {
                "role": "system",
                "content": "你是一个电子元器件销售策略专家，擅长将新产品匹配给最可能采购的客户。",
            },
            {
                "role": "user",
                "content": product_customer_matching_prompt(
                    product_profile, candidates_text
                ),
            },
        ],
        schema,
