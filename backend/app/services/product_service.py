"""Product service — list / stats / detail / sales aggregations.

Stage 1 refactor: extracts business logic from
``api/v1/products/list.py`` (660 lines) into a dedicated service so the
route layer becomes a thin proxy.

Covers four endpoints:
- ``GET /products/stats/summary``   dashboard KPIs
- ``GET /products``                 paginated list (with cache)
- ``GET /products/{id}``            detail with joined metrics
- ``GET /products/{id}/sales``      cross-document sales history

The metrics subqueries (inventory / supplier / sales) and the row
serialization stay close to the read path so the SQL remains auditable.
"""
...
# ── Pure row formatters (no DB) ────────────────────────────────────────


def product_completion(p: Product) -> tuple[int, list[str]]:
    """Compute product completion score (0-100) and missing field labels."""
    fields = [
        ("SKU", p.sku),
        ("MPN", p.mpn),
        ("品牌", p.brand_id),
...
def stock_status_for(available: int | None, safety_stock: int | None) -> str:
    """Translate available / safety stock into a coarse stock status."""
...
def product_row(
    ...
    *,
    ...
    return {
        ...
        "stock_status": stock_status_for(available, safety_stock),
...
def products_cache_key(**parts: object) -> str:
    """Stable cache key derived from query parameters."""
...
class ProductService(BaseCRUDService):
    """Product read paths (list, detail, stats, sales history).

    CRUD for the standard create/update/delete lives in
    ``services/product_crud_service.py`` and the route layer in
    ``api/v1/products/crud.py``.
    """

    model = Product

    async def get_stats_summary(self, db: AsyncSession) -> dict[str, Any]:
    ...
    async def list_products(
        ...
        *,
        ...
        """Paginated product list with filters / scenes / sort."""
        ...
        # Optional cache lookup — imported lazily to keep the service
        # importable in tests where the cache backend is absent.
    ...
    async def get_product_detail(
        ...
        """Single product with joined inventory/supplier/sales metrics."""
        inv_subq = _inventory_metrics_subquery()
        sales_subq = _sales_metrics_subquery()
        supplier_subq = _supplier_metrics_subquery()
    ...
    async def get_product_sales_history(
        ...
        """Recent quotations, orders, and deliveries that reference this product."""
