...
async def get_supplier_scorecard(db: AsyncSession, supplier_id: int) -> dict:
    ...
    on_time_rate = (
        round(on_time_count / total_completed * 100, 1) if total_completed > 0 else None
    )
    avg_lead_time = (
        round(promised_lead_time_days_total / total_completed, 1)
    ...
    try:
        result = await ai_client.chat_structured(
            [
                {
                    "role": "system",
                    "content": "你是一个电子元器件供应链管理专家，擅长供应商绩效评估和评分卡设计。",
                },
...
async def predict_supplier_delay(db: AsyncSession, supplier_id: int) -> dict:
    ...
    for po in recent_pos:
        if po[1] in ("completed", "received", "delivered") and po[2] and po[3]:
            ...
            last_delivery_date = (
                po[2] if last_delivery_date is None else max(last_delivery_date, po[2])
            )

    avg_delay_days = (
        round(total_delay_days / recent_delays, 1) if recent_delays > 0 else 0
    )
    ...
    early_total = sum(
        1
        for po in recent_pos
        if po[1] in ("completed", "received", "delivered") and po[3] < three_months_ago
    )
    late_total = sum(
        1
        for po in recent_pos
    ...
    early_delays = sum(
        ...
        and po[2]
        and po[3]
        and (po[2] - po[3]).days < 0
        and po[3] < three_months_ago
    )
    late_delays = sum(
        1
        for po in recent_pos
    ...
    pending_pos = (
        await db.execute(
            select(
                func.count(PurchaseOrder.id), func.sum(PurchaseOrder.total_amount)
            ).where(
                PurchaseOrder.supplier_id == supplier_id,
    ...
    delay_data = {
        ...
        "last_delivery_date": last_delivery_date.strftime("%Y-%m-%d")
    ...
    try:
        result = await ai_client.chat_structured(
            [
                {
                    "role": "system",
                    "content": "你是一个电子元器件供应链风险分析师，擅长预测交付延迟和制定缓解方案。",
                },
                {
                    "role": "user",
                    "content": supplier_delay_prediction_prompt(delay_data),
                },
            ],
            schema,
        )
        ...
        candidate_rows = (
            await db.execute(
                select(
                    ...
                    func.count(func.distinct(SupplierProduct.product_id)).label(
                        "overlap_count"
                    ),
            ...
            candidates.append(
                {
                    ...
                    "avg_lead_time": f"{round(float(r[5]), 1)}天"
                    if r[5] is not None
                    else "无数据",
                }
            )
    ...
    try:
        result = await ai_client.chat_structured(
            [
                {
                    "role": "system",
                    "content": "你是一个电子元器件供应链优化专家，擅长供应商替代分析和多元化策略。",
                },
...
async def detect_supplier_price_variance(db: AsyncSession, supplier_id: int) -> dict:
    ...
    if current_price and price_3m_ago and float(price_3m_ago) > 0:
        # 短期趋势：近3个月 vs 3-6个月前
        change = (
            (float(current_price) - float(price_3m_ago)) / float(price_3m_ago) * 100
        )
        ...
        change = (
            (float(current_price) - float(price_6m_ago)) / float(price_6m_ago) * 100
        )
    ...
    if current_price and peer_catalog_price and float(peer_catalog_price) > 0:
        diff = (
            (float(current_price) - float(peer_catalog_price))
            / float(peer_catalog_price)
            * 100
        )
    ...
    if current_product_ids := product_ids:
        ...
        sp_costs = dict(
            (
                await db.execute(
                    select(
                        SupplierProduct.product_id, SupplierProduct.cost_price
                    ).where(
                        SupplierProduct.supplier_id == supplier_id,
                        SupplierProduct.product_id.in_(current_product_ids),
                        SupplierProduct.cost_price.isnot(None),
    ...
    variance_data = {
        "name": supplier.name,
        "current_avg_price": f"¥{float(current_price):.4f}"
        if current_price
        else "无数据",
        ...
        "market_benchmark": f"¥{float(peer_catalog_price):.4f}"
        if peer_catalog_price
        else "无市场基准数据（需同行 PO 成交价）",
        "peer_avg_price": f"¥{float(peer_catalog_price):.4f}"
        if peer_catalog_price
        else "无数据（需同行目录价）",
    ...
    try:
        result = await ai_client.chat_structured(
            [
                {
                    "role": "system",
                    "content": "你是一个电子元器件采购成本分析师，擅长价格异常检测和降本策略。",
                },
                {
                    "role": "user",
                    "content": supplier_price_variance_prompt(variance_data),
                },
            ],
            schema,
        )
        ...
        logging.getLogger(__name__).warning(
            f"Price variance AI failed for supplier {supplier_id}: {e}"
        )
        result = {
            "price_status": "正常",
            "variance_score": 50,
...
async def get_supplier_360(db: AsyncSession, supplier_id: int) -> dict:
    ...
    for po in pos:
        if po.expected_date and po.created_at:
            d = (
                (po.expected_date - po.created_at).days
                if hasattr(po.expected_date - po.created_at, "days")
                else 0
            )
    ...
    po_history = {
        ...
        "avg_delivery_days": round(sum(delivery_days) / len(delivery_days), 1)
        if delivery_days
        else 0,
    ...
    context_data = {
        ...
        "linked_product_count": (
            await db.execute(
                select(func.count(SupplierProduct.product_id)).where(
                    SupplierProduct.supplier_id == supplier_id
                )
            )
        ).scalar()
    ...
    try:
        ai_result = await ai_client.chat_structured(
            [
                {
                    "role": "system",
                    "content": "你是一个电子元器件分销行业供应商管理专家。返回简洁有效的JSON。",
                },
                {
                    "role": "user",
                    "content": (
...
async def compare_suppliers(db: AsyncSession, supplier_ids: list[int]) -> dict:
    ...
    supplier_rows = await db.execute(
        select(Supplier).where(
            Supplier.id.in_(supplier_ids), Supplier.deleted_at.is_(None)
        )
    )
    ...
    for sid in supplier_ids:
        ...
        cert_score = (
            20
            if (supplier.certifications and supplier.certifications not in ("无", ""))
            else 5
        )
        fin_score = {"A": 20, "B": 15, "C": 10, "D": 5}.get(
            supplier.financial_rating or "C", 10
        )
    ...
    for s in suppliers_data:
        dimensions[0]["scores"][s["name"]] = (
            20 if s["certifications"] and s["certifications"] != "无" else 5
        )
        dimensions[1]["scores"][s["name"]] = {"A": 20, "B": 15, "C": 10, "D": 5}.get(
            s["financial_rating"], 10
        )
        dimensions[2]["scores"][s["name"]] = min(s["po_count"] * 3, 20)
        dimensions[3]["scores"][s["name"]] = min(s["linked_products"] * 4, 20)
        dimensions[4]["scores"][s["name"]] = (
            20 if s["type"] in ("授权分销商", "原厂") else 12
        )
    ...
    if sp_count > 0:
        ...
        if sp_product_ids:
            alt_count = (
                await db.execute(
                    select(
                        func.count(func.distinct(SupplierProduct.supplier_id))
                    ).where(
    ...
    try:
        ai_result = await ai_client.chat_structured(
            [
                {
                    "role": "system",
                    "content": "你是一个电子元器件采购谈判专家。返回简洁有效的JSON。",
                },
