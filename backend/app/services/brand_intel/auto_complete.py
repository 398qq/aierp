...
async def auto_complete_brand(db: AsyncSession, brand_id: int) -> dict:
    ...
    missing_fields = [
        k for k, v in existing.items() if v is None and k not in ("short_name", "logo")
    ]
    ...
    try:
        text_out = await ai_client.chat(
            [
                {
                    "role": "system",
                    "content": "你是电子元器件行业专家。只返回指定格式的数据，不要解释。",
                },
        ...
        for line in text_out.strip().split("\n"):
            ...
            val = val.strip()
            if (
                key in missing_fields
                and val
                and val not in ("None", "none", "null", "未知", "无", "-")
            ):
                filled[key] = val
                if key == "risk_score":
                    try:
        ...
        return {
            "brand_id": brand_id,
            "filled": filled,
            "message": f"已补全 {len(filled)} 个字段"
            if filled
            else "AI 未能补全任何字段，可能是缺失字段信息不足",
        }
