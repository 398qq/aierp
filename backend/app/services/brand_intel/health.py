...
async def get_brand_health(db: AsyncSession, brand_id: int) -> dict:
    ...
    if product_ids:
        ...
        monthly_revenue = (
            ", ".join(
                f"{r[0].strftime('%Y-%m')}: ¥{float(r[1]):,.0f}" for r in monthly_rows
            )
            if monthly_rows
            else "无"
        )
        ...
        revenue_growth = (
            f"{((float(r3m) - float(r3m_prev)) / float(r3m_prev) * 100):.1f}"
            if float(r3m_prev) > 0
            else "无数据"
        )
        ...
        cost_rows = (
            await db.execute(
                ...
                .join(
                    SupplierProduct,
                    SalesOrderItem.product_id == SupplierProduct.product_id,
                )
                .where(
                    SalesOrderItem.product_id.in_(product_ids),
                    SalesOrder.status.in_(["confirmed", "delivered", "completed"]),
        ...
        margin_pct = (
            f"{((total_revenue - float(cost_rows)) / total_revenue * 100):.1f}"
            if total_revenue > 0
            else "无数据"
        )
    ...
    result = await ai_client.chat_structured(
        [
            {
                "role": "system",
                "content": "你是一个电子元器件供应链分析专家，擅长品牌经营健康度评估。",
            },
