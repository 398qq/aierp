...
async def predict_brand_lifecycle(db: AsyncSession, brand_id: int) -> dict:
    ...
    if product_ids:
        ...
        revenue_growth = (
            f"{((float(rev_12m) - float(rev_prev)) / float(rev_prev) * 100):.1f}"
            if float(rev_prev) > 0
            else "无历史数据"
        )
    ...
    lc_data = {
        **ctx,
        ...
        "supplier_trend": "无数据",
        "product_intro_rhythm": f"近6月{new_6m}新品"
        if new_6m > 0
        else "近期无新品推出",
    }
    ...
    result = await ai_client.chat_structured(
        [
            {
                "role": "system",
                "content": "你是一个电子元器件产品生命周期管理专家，擅长判断品牌所处阶段并给出战略建议。",
            },
...
async def get_brand_price_trends(db: AsyncSession, brand_id: int) -> dict:
    ...
    if product_ids:
        ...
        if price_rows:
            ...
            if len(price_rows) > 1:
                ...
                delta = (
                    (float(price_rows[-1][1]) - float(price_rows[0][1]))
                    / float(price_rows[0][1])
                    * 100
                )
    ...
    result = await ai_client.chat_structured(
        [
            {
                "role": "system",
                "content": "你是一个电子元器件定价策略专家，擅长价格趋势分析和定价优化。",
            },
