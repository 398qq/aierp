...
async def scan_eol_alerts(db: AsyncSession, urgency_threshold: str = "warning") -> dict:
    ...
    for brand in at_risk_brands:
        ...
        sales_q = (
            await db.execute(
                select(func.coalesce(func.sum(SalesOrderItem.total_price), 0)).where(
                    SalesOrderItem.product_id.in_(prod_ids),
        ...
        stage_label = (
            "已停产(EOL)" if brand.lifecycle_stage == "eol" else "不推荐新设计(NRND)"
        )
        severity = (
            "critical"
            if brand.lifecycle_stage == "eol" and float(sales_q) > 0
            else "warning"
        )
    ...
    return {
        ...
        "alerts": alerts,
        "summary": (
            f"发现 {total} 个EOL/NRND品牌风险（{critical} 紧急，{warning} 关注）"
            if total > 0
            else "未发现EOL/NRND风险"
        ),
    }
...
async def suggest_eol_alternatives(db: AsyncSession, product_id: int) -> dict:
    ...
    if product.brand_id:
        brand = (
            (await db.execute(select(Brand).where(Brand.id == product.brand_id)))
            .scalars()
            .first()
        )
    ...
    active_brands = (
        (await db.execute(select(Brand).where(*cond).limit(5))).scalars().all()
    )
