...
async def get_brand_supplier_matrix(db: AsyncSession, brand_id: int) -> dict:
    ...
    supplier_price_ranking = (
        ", ".join(
            f"{s['supplier_name']}: ¥{s['avg_cost']:.4f}"
            if s["avg_cost"]
            else f"{s['supplier_name']}: 暂无报价"
    ...
    backup_pct = (
        round(products_with_backup / len(product_ids) * 100, 1) if product_ids else 0
    )
    ...
    matrix_data = {
        **ctx,
        ...
        "avg_products_per_supplier": round(
            ctx.get("product_count", 0) / max(len(supplier_details), 1), 1
        ),
    ...
    result = await ai_client.chat_structured(
        [
            {
                "role": "system",
                "content": "你是一个电子元器件采购策略专家，擅长供应商矩阵分析和采购优化。",
            },
