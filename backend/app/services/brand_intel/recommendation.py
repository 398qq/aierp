...
async def recommend_brands(db: AsyncSession, brand_id: int, top_k: int = 5) -> dict:
    ...
    if not customer_ids:
        return {
            "recommendation_summary": "暂无购买客户，无法生成推荐",
            "recommended_brands": [],
            "context": ctx,
        }
    ...
    co_purchase_rows = (
        await db.execute(
            select(
                ...
                Brand.name_cn,
                Brand.category,
                func.count(func.distinct(SalesOrder.customer_id)).label(
                    "shared_customers"
                ),
    ...
    if not co_purchase_rows:
        return {
            "recommendation_summary": "未发现关联购买品牌",
            "recommended_brands": [],
            "context": ctx,
        }
    ...
    result = await ai_client.chat_structured(
        [
            {
                "role": "system",
                "content": "你是一个电子元器件销售策略专家，擅长交叉销售和品牌推荐。",
            },
