...
class QuotationService(BaseCRUDService):
    """Quotation service.

    Stage 1: business methods are class methods on this service. Module-level
    functions below are kept as thin proxies for back-compat with existing
    routes and tests.
    """
    ...
    async def send_quotation(self, db: AsyncSession, quote: Quotation) -> Quotation:
        ...
        await db.refresh(quote)
        try:
            await self._notify_quotation_sent(quote)
    ...
    async def _notify_quotation_sent(self, quote: Quotation) -> None:
...
async def get_quotation(db: AsyncSession, quote_id: int) -> Quotation | None:
    return await quotation_service.get_quotation(db, quote_id)
...
async def get_quotation_stats(db: AsyncSession) -> dict:
    return await quotation_service.get_quotation_stats(db)


async def duplicate_quotation(db: AsyncSession, quote: Quotation) -> Quotation:
    return await quotation_service.duplicate_quotation(db, quote)
...
async def create_quotation_from_inquiry(
    ...
    *,
    ...
    return await quotation_service.create_quotation_from_inquiry(
        db,
...
async def _notify_quotation_sent(quote: Quotation) -> None:
    """Back-compat proxy for the WeCom notifier (also exposed as class method)."""
    await quotation_service._notify_quotation_sent(quote)
