...
async def generate_visit_report(db: AsyncSession, visit_id: int) -> dict:
    ...
    try:
        result = await ai_client.chat_structured(messages, output_schema)
    except ValueError as e:
        logging.getLogger(__name__).warning(f"generate_visit_report AI failed: {e}")
        result = {
            "visit_summary": "AI分析暂时不可用",
            "key_achievements": [],
            "customer_sentiment": "中性",
            "engagement_level": "中",
            "product_interest": "AI分析暂时不可用",
            "opportunity_signals": [],
            "risk_signals": [],
            "action_items": [],
            "followup_recommendation": "AI分析暂时不可用",
...
async def analyze_visit_sentiment(db: AsyncSession, visit_id: int) -> dict:
    ...
    try:
        ...
        logging.getLogger(__name__).warning(f"analyze_visit_sentiment AI failed: {e}")
        result = {
...
async def evaluate_visit_effectiveness(db: AsyncSession) -> dict:
    ...
    try:
        ...
        ai_result = {
            ...
            "productivity_assessment": "AI分析暂时不可用",
            "high_performers": [],
            "gaps": [],
            "optimization_suggestions": [],
            "visit_frequency_recommendation": "AI分析暂时不可用",
        }
