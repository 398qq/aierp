...
def generate_basic_pdf(quotation: Any, options: dict[str, Any] | None = None) -> bytes:
    ...
    if opts["show_terms"]:
        terms = opts["terms"].splitlines() if opts["terms"] else [
    ...
    if opts["show_signature"]:
        lines.extend(["", "Prepared by: " + (opts["prepared_by"] or "-"), "Customer signature: __________________"])
...
def generate_quotation_pdf(quotation: Any, options: dict[str, Any] | None = None) -> bytes:
    ...
    created_at = date_text(getattr(quotation, "created_at", None) or datetime.now(timezone.utc))
    ...
    header_table = Table(
        [
            [
                Paragraph(opts["company_name"], ParagraphStyle(
                    "HeaderCompany",
                    parent=title_style,
                    alignment=0,
                    textColor=colors.white,
    ...
    if items:
        ...
        if internal:
            table_style.append(("TEXTCOLOR", (-1 if not opts["show_line_hints"] else -2, 1), (-1 if not opts["show_line_hints"] else -2, -1), colors.HexColor("#166534")))
        ...
        totals_data = [
            ...
            [Paragraph("报价合计", total_style), Paragraph(money(quote_total), total_style)],
            [Paragraph("人民币大写", label_style), Paragraph(money_upper_cn(quote_total), normal_style)],
        ]
        if abs(variance) >= Decimal("0.01"):
            totals_data.insert(1, [Paragraph("调整差额", label_style), Paragraph(money(variance), normal_style)])
        if internal:
            totals_data.extend([
                [Paragraph("未税成本", label_style), Paragraph(money(untaxed_cost), normal_style)],
                [Paragraph("含税成本", label_style), Paragraph(money(taxed_cost), normal_style)],
                [Paragraph("销售毛利 / 毛利率", total_style), Paragraph(f"{money(profit)} / {percent(margin)}", total_style)],
            ])
