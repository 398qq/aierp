...
async def list_delivery_notes(
    ...
    if notes:
        ...
        if so_ids:
            sos = {s.id: s for s in (await db.execute(select(SalesOrder).where(SalesOrder.id.in_(so_ids)))).scalars().all()}
        item_rows = (await db.execute(
            select(DeliveryNoteItem)
            .where(DeliveryNoteItem.delivery_note_id.in_([n.id for n in notes]), DeliveryNoteItem.deleted_at.is_(None))
        )).scalars().all()
