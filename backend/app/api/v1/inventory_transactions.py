...
async def list_inventory_transactions(
    ...
    type: str | None = Query(None, description="Transaction type: stock_in, stock_out, adjust, transfer"),
    ...
    count_base = select(func.count(InventoryTransaction.id)).where(InventoryTransaction.deleted_at.is_(None))
    ...
    rows = (await db.execute(
        base.order_by(InventoryTransaction.created_at.desc()).offset((page - 1) * page_size).limit(page_size)
    )).all()
