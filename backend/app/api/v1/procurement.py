...
async def restock_suggest(
    ...
    q = select(
        Inventory, Product.name, Product.sku,
    ...
    for inv, prod_name, sku in rows:
        ...
        # Find best supplier
        sp = (await db.execute(
            select(SupplierProduct, Supplier.name).join(Supplier, SupplierProduct.supplier_id == Supplier.id).where(
                SupplierProduct.product_id == inv.product_id,
                SupplierProduct.deleted_at.is_(None),
                Supplier.deleted_at.is_(None),
            ).order_by(SupplierProduct.cost_price.asc().nulls_last()).limit(1)
        )).first()
        ...
        suggestions.append({
            ...
            "gap": gap,
            "suggested_order": max(gap, int(inv.safety_stock * 0.5) if inv.safety_stock else 10),
            "best_supplier": {"id": sp[0].supplier_id, "name": sp[1], "cost_price": float(sp[0].cost_price or 0)} if sp else None,
        })
...
async def supplier_recommend(
    ...
    for pid in ids:
        product = (await db.execute(
            select(Product.name, Product.sku).where(Product.id == pid, Product.deleted_at.is_(None))
        )).first()
        if not product:
            continue

        suppliers = (await db.execute(
            select(SupplierProduct, Supplier).join(Supplier, SupplierProduct.supplier_id == Supplier.id).where(
        ...
        for sp, sup in suppliers:
            ...
            options.append({
                ...
                "moq": sp.moq, "is_preferred": sp.is_preferred,
                "score": score,
            })

        options.sort(key=lambda x: x["score"], reverse=True)
        recommendations.append({
            "product_id": pid, "product_name": product[0], "sku": product[1],
            "options": options[:5],
        })
...
async def procurement_dashboard(
    ...
    ontime_rate = round(ontime_pos / total_delivered * 100, 1) if total_delivered > 0 else 0

    return ok({
        "total_po": total_po,
        "total_amount": float(total_amount),
        "status_distribution": [{"status": s, "count": c} for s, c in po_status],
        "monthly_trend": [{"month": m, "count": c, "amount": float(a)} for m, c, a in monthly],
        "on_time_delivery": {"rate": ontime_rate, "on_time": ontime_pos, "late": late_pos},
    })
...
async def po_calendar(
    ...
    return ok([{
        ...
        "total_amount": float(po.total_amount), "status": po.status,
