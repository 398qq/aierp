...
def _card_ocr_key_hits(text: str) -> dict[str, bool]:
    normalized = _normalize_ocr_text(text)
    return {
...
def _card_ocr_selection_rank(item: dict[str, Any]) -> tuple[int, int, float]:
    hits = _card_ocr_key_hits(str(item.get("text") or ""))
    ...
    return phone_email_hits, key_hits, primary_engine_bonus, float(item.get("score") or 0)


def _is_high_signal_ocr_line(line: str) -> bool:
...
def _merge_high_signal_ocr_lines(best_text: str, candidates: list[dict[str, Any]]) -> str:
    merged_lines = _normalize_ocr_text(best_text).splitlines()
    seen = {line.lower() for line in merged_lines if line.strip()}

    for item in sorted(candidates, key=lambda item: _card_ocr_selection_rank(item), reverse=True):
        for line in _normalize_ocr_text(str(item.get("text") or "")).splitlines():
            normalized_line = line.strip()
            if not normalized_line or normalized_line.lower() in seen:
                continue
            if not _is_high_signal_ocr_line(normalized_line):
                continue
...
def _business_card_image_variants(image: Any) -> list[tuple[str, Any]]:
    ...
    variants.extend(_opencv_business_card_region_variants(base))
...
def _opencv_business_card_region_variants(image: Any) -> list[tuple[str, Any]]:
    ...
    for canny_low, canny_high in canny_thresholds:
        ...
        for kernel_name, kernel in kernels.items():
            ...
            for contour in contours:
                ...
                # Standard business cards are close to 1.75:1. Prefer larger, centered candidates.
...
def _ocr_with_easyocr(image: Any) -> dict[str, Any]:
    try:
        import numpy as np
    except Exception as exc:
...
def _extract_business_card_ocr(content: bytes) -> dict[str, Any]:
    ...
    try:
        ...
        variants = _business_card_image_variants(image)
        candidates: list[dict[str, Any]] = []
        easyocr_variants = [
            (name, variant)
        ...
        for variant_name, variant in easyocr_variants:
            try:
                easyocr_result = _ocr_with_easyocr(variant)
