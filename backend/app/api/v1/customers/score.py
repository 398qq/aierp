    ...
    for customer in customers:
        try:
            ...
            recency, frequency, monetary, tier = rfm_bucket(
                days_since_contact, order_count, total_amount
            )
            ...
            health_score = max(
                0.0,
                min(100.0, round(46.0 + level_bonus + contact_term + order_term, 1)),
            )
            ...
            churn_risk = max(
                0.0,
                min(
                    100.0, round(order_risk * 0.55 + contact_risk * 0.35 + opp_risk, 1)
                ),
            )
