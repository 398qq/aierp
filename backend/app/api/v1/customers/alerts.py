    ...
    rows = (
        (
            await db.execute(
                ...
                .order_by(AlertRule.rule_type, AlertRule.name)
            )
        )
        .scalars()
        .all()
...
async def list_alert_events(
    ...
    count_base = select(func.count(AlertEvent.id)).where(
    ...
    rows = (
        await db.execute(
            base.order_by(AlertEvent.created_at.desc())
    ...
    return ok(
        {
            "list": [
                {
                    "id": e.AlertEvent.id,
                    "customer_id": e.AlertEvent.customer_id,
                    "customer_name": e.customer_name,
                    "rule_type": e.AlertEvent.rule_type,
                    "rule_name": e.AlertEvent.rule_name,
                    "severity": e.AlertEvent.severity,
                    "message": e.AlertEvent.message,
                    "is_read": e.AlertEvent.is_read,
                    "read_at": str(e.AlertEvent.read_at)
                    if e.AlertEvent.read_at
                    else None,
                    "created_at": str(e.AlertEvent.created_at)
                    if e.AlertEvent.created_at
                    else None,
                }
                for e in rows
            ],
...
async def mark_alert_read(
    ...
    result = await db.execute(
        select(AlertEvent).where(
            AlertEvent.id == event_id, AlertEvent.deleted_at.is_(None)
        )
    )
...
async def mark_all_alerts_read(
    ...
    rows = (
        (
            await db.execute(
                select(AlertEvent).where(
                    ~AlertEvent.is_read, AlertEvent.deleted_at.is_(None)
                )
            )
        )
        .scalars()
...
async def get_alert_events(
    ...
    rows = (
        await db.execute(
            ...
            .where(
                AlertEvent.deleted_at.is_(None),
                AlertEvent.rule_type
                == select(AlertRule.rule_type)
        ...
        (
            await db.execute(
                select(AlertRule).where(
                    AlertRule.enabled, AlertRule.deleted_at.is_(None)
                )
            )
        )
        .scalars()
        ...
        (
            await db.execute(
                select(Payment).where(
                    Payment.deleted_at.is_(None), Payment.paid_at.is_(None)
                )
            )
        )
        .scalars()
        ...
        for rule in rules:
            ...
            if rule.rule_type == "no_order" and rule.threshold_days:
                ...
                if latest_order:
                    ...
                    c.created_at
                    and (now - (c.created_at or now).replace(tzinfo=timezone.utc)).days
                    >= rule.threshold_days
                ):
                ...
                for p in c_payments:
                    overdue_days = (
                        now - p.created_at.replace(tzinfo=timezone.utc)
                    ).days
                ...
                prior = sum(
                    ...
                    for o in c_orders
                    if 90
                    < (now - o.created_at.replace(tzinfo=timezone.utc)).days
                    <= 180
                )
