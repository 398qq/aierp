...
# Step 1: Login
r = subprocess.run(['curl', '-s', '-X', 'POST', 'http://localhost:8080/api/v1/auth/login',
...
# Step 2: Trigger scan
r2 = subprocess.run(['curl', '-s', '-X', 'GET', 'http://localhost:8080/api/v1/ai/watchtower/scan?days_back=90',
    '-H', f'Authorization: Bearer {token}'],
    capture_output=True, text=True)
print(f"Scan response: {r2.stdout[:800]}", file=sys.stderr)

# Step 3: Get unread alerts
r3 = subprocess.run(['curl', '-s', '-X', 'GET', 'http://localhost:8080/api/v1/customers/alerts?is_read=false&page_size=10',
    '-H', f'Authorization: Bearer {token}'],
    capture_output=True, text=True)
